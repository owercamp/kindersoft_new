          ...
          <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" wire:key="doc-{{ $quote->id }}">
              ...
              <td scope="row" class="flex justify-around justify-items-center">
                ...
                <x-button class="bg-blue-800 hover:bg-blue-700 my-2 flex flex-row" wire:click="whatsApp({{ $quote->id }})" wire:loading.class="opacity-50" wire:loading.attr="disabled">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 mr-1">
                  ...
                  <div wire:loading.class="hidden" wire:target="whatsApp({{ $quote->id }})">
                    {{ __('Send') }} WhatsApp
                  </div>
                  <div wire:loading wire:target="whatsApp({{ $quote->id }})">
                    {{ __('Consulting...') }}
                  </div>
                </x-button>
              </td>
