<div>
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight text-end px-4">
      {{ __('Admissions') }} > {{ __('Commercial Proposal') }} > {{ __('Customers') }}
    </h2>
  </x-slot>
  <div class="py-8">
      ...
      <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg py-6 px-5">
        <div class="flex flex-row mt-3">
          <div class="w-full">
            ...
            <div x-data="{ modal: $wire.entangle('modal_quote').live }">
              <div class="fixed inset-0 z-50 flex items-center justify-center overflow-auto bg-black bg-opacity-50" x-show="modal" id="edit">
                <!-- Modal inner -->
                <div class="max-w-5xl max-h-[42rem] px-6 py-4 mx-auto text-left text-gray-800 dark:text-gray-200 bg-white dark:bg-gray-800 rounded shadow-lg w-9/12 overflow-y-auto" @click.away="showModal = false" x-transition:enter="motion-safe:ease-out duration-300" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100">
                  <!-- Title / Close-->
                  <div class="flex items-center justify-between my-4">
                    <h5 class="mr-3 text-gray-800 dark:text-gray-200 max-w-none">{{ __('Quote') }}</h5>
                  ...
                  <!-- content -->
                  <div class="p-4">
                    <form wire:submit.prevent="quotation">
                      @csrf
                      @method('PUT')
                      @include('livewire.admissions.commercial-proposal.partials.modal-quote')
                      <div class="flex justify-end mt-1">
                        <x-button class="bg-green-800 hover:bg-green-700 dark:bg-sky-800 dark:hover:bg-sky-600 w-full" type="submit">{{ __('Save') }}</x-button>
                      </div>
                    </form>
              ...
              <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" wire:key="doc-{{ $potential->id }}">
                  ...
                  <td scope="row" class="flex justify-around justify-items-center">
                    ...
                    <x-button class="bg-green-800 hover:bg-green-700 my-2 flex flex-row" wire:click="openEdit({{ $potential->customer_client->id }})" wire:loading.class="opacity-50" wire:loading.attr="disabled">
                    ...
                    <x-button class="bg-blue-800 hover:bg-blue-700 my-2 flex flex-row" wire:click="openQuote({{ $potential->id }})" wire:loading.class="opacity-50" wire:loading.attr="disabled">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 mr-1">
                      ...
                      <div wire:loading.class="hidden" wire:target="openQuote({{ $potential->id }})">
                        {{ __('Quote') }}
                      </div>
                      <div wire:loading wire:target="openQuote({{ $potential->id }})">
                        {{ __('Consulting...') }}
                      </div>
                    </x-button>
