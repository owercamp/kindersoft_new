<div>
  <div class="flex flex-row gap-4 mb-4 w-full">
    <div class="max-w-[9rem]">
      ...
      <x-input wire:model="registerForm.register" disabled type="text" class="mt-1 block w-full" />
      <x-input-error for="registerForm.register" />
    </div>
    <div class="min-w-[10rem]">
      <x-label style="text-transform: capitalize;">{{ __('validation.attributes.date') }}</x-label>
      <x-input type="date" class="mt-1" wire:model="registerForm.date" />
      <x-input-error for="registerForm.date" />
    </div>
    <div class="w-full">
      <x-label>{{ __('Name') }} {{ rtrim(__('Attendees'), 's') }}</x-label>
      <x-input type="text" class="mt-1 block w-full" wire:model="registerForm.name" />
  ...
  <div class="flex flex-row gap-4 mb-4 w-full">
    <div class="max-w-[10rem]">
      ...
      <x-input type="text" class="mt-1 block w-full" wire:model="registerForm.phone" />
      <x-input-error for="registerForm.phone" />
    </div>
    <div class="max-w-[10rem]">
      <x-label> WhatsApp</x-label>
      <x-input type="text" class="mt-1 block w-full" wire:model="registerForm.whatsapp" />
      <x-input-error for="registerForm.whatsapp" />
    </div>
    <div class="w-full">
      <x-label>{{ __('Email') }}</x-label>
      <x-input type="email" class="mt-1 block w-full" wire:model="registerForm.email" />
  ...
  <div class="flex flex-row gap-4 mb-4 w-full">
    <div class="max-w-[9rem]">
      ...
      <x-input type="number" class="mt-1 block w-full" wire:model="registerForm.applicants" />
      <x-input-error for="registerForm.applicants" />
    </div>
    <div class="w-full">
      <x-label>{{ __('Name') }} {{ __('Applicant') }}</x-label>
      <x-input type="text" class="mt-1 block w-full" wire:model="registerForm.applicant" />
  ...
  <div class="flex flex-row gap-4 mb-4 w-full">
    ...
    <div class="max-w-[10rem]">
      ...
      <x-input type="date" class="mt-1" wire:model="registerForm.birthday" wire:change="calculateAge" />
